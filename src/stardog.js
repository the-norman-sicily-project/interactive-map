...
const stardogConnection = new Connection({
  ...
  endpoint: `http://${config.stardog.host}:${config.stardog.port}`,
});
...
const executeGraphqlQuery = async (q, v = {}) => {
  try {
    const response = await query.graphql.execute(stardogConnection, config.stardog.database, q, v, { reasoning: true });
    if (!response.ok) {
...
export const fetchPlaces = async () => executeGraphqlQuery(placesQuery);

export const fetchPlace = async (action) => {
  const { placeId, placeType } = action.place;
...
`;
  ...
  return executeGraphqlQuery(placeQuery, { placeId, placeType });
};
